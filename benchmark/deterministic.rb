...
SHARED_INPUTS = Integer(ENV.fetch("DETERMINISTIC_SHARED_INPUTS", "0"))
raise "shared inputs must be nonnegative" if SHARED_INPUTS.negative?
...
if SHARED_INPUTS.positive?
  FileUtils.mkdir_p(File.join(PROJECT, "config"))
  SHARED_INPUTS.times do |index|
    File.write(File.join(PROJECT, "config/setting_#{index}.json"), JSON.generate({name: "setting_#{index}", weight: index + 1}))
  end
  File.write(File.join(PROJECT, "testmon_config.rb"), <<~RUBY)
    Minitest::Testmon.configure do |configuration|
      configuration.provider :settings, version: 1 do |builder|
        builder.inventory :settings, root: :project, include: "config/*.json"
        builder.facet :values, inventory: :settings, digest: :content, granularity: :file, scope: :suite
        builder.facet :membership, inventory: :settings, digest: :paths, granularity: :set, scope: :suite
      end
    end
  RUBY
end
  ...
  File.write(File.join(PROJECT, "test/work_#{index}_test.rb"), <<~RUBY)
    class Work#{index}Test < Minitest::Test
      def test_payload
        ...
        #{'assert_equal Integer(ENV.fetch("DETERMINISTIC_CONFIG_TOTAL")), CONFIG_TOTAL' if SHARED_INPUTS.positive?}
      end
    end
  RUBY
end

def run_child(label, enabled:, cache:, config_total: SHARED_INPUTS * (SHARED_INPUTS + 1) / 2)
  ...
  environment = {"MINITEST_TESTMON" => enabled ? "1" : "0", "MINITEST_TESTMON_DB" => cache,
                 ...
                 "MINITEST_TESTMON_CONFIG" => nil, "DETERMINISTIC_CONFIG_TOTAL" => nil}
  if SHARED_INPUTS.positive?
    environment["MINITEST_TESTMON_CONFIG"] = File.join(PROJECT, "testmon_config.rb")
    environment["DETERMINISTIC_CONFIG_TOTAL"] = config_total.to_s
  end
  ...
  if enabled
    ...
    result[:suite_members_per_set] = db.execute(<<~SQL).map(&:first)
      SELECT count(*) FROM suite_input_set_members
      WHERE set_id IN (SELECT DISTINCT suite_input_set_id FROM test_snapshots)
      GROUP BY set_id ORDER BY count(*)
    SQL
...
results << accessor
if SHARED_INPUTS.positive?
  path = File.join(PROJECT, "config/setting_0.json")
  setting = JSON.parse(File.read(path))
  setting["weight"] += 1
  File.write(path, JSON.generate(setting))
  shared = run_child("shared-config-change", enabled: true, cache: cache,
    config_total: SHARED_INPUTS * (SHARED_INPUTS + 1) / 2 + 1)
  raise "shared config change did not select every test" unless shared[:selected].length == TESTS
  results << shared
end
summary = {ruby: RUBY_DESCRIPTION, seed: SEED, tests: TESTS, iterations: ITERATIONS, sources: 121,
           shared_configuration_inputs: SHARED_INPUTS,
